...
include::doc/book/installing/_docker-for-maven-tutorials.adoc[]
...
=== Create your Multibranch Pipeline project in Jenkins
...
. Scroll down and select *Multibranch Pipeline*, then select *OK* at the end of the page.
. (Optional) Enter a Pipeline *Description*.
. Select *Pipeline* on the left pane.
. Click on *Add source* in the *Branch Sources* section below the description, and select *Git*.
. Paste in the link of your forked repo, same one you used to clone the repo `https://github.com/YOUR-GITHUB-ACCOUNT-NAME/simple-java-maven-app`
. Click "Save" at the bottom to save your new Multibranch Pipeline project. This will start scanning your forked repo and should show `Scan Multibranch Pipeline Log` as `SUCCESS`
. You're now ready.
...
Your Pipeline is created as a `Jenkinsfile`, which is pushed to your forked Git repository (`simple-java-maven-app`).
...
----
<1> This uses the angent container to execute the jenkinsfile 
<2> Defines a link:/doc/book/pipeline/syntax/#stage[`stage`] (directive) called `Build` that appears on the Jenkins UI.
<3> This link:/doc/pipeline/steps/workflow-durable-task-step/#code-sh-code-shell-script[`sh`] step runs the Maven command to cleanly build your Java application without running any tests.
. Save your edited `Jenkinsfile` and push it to your forked `simple-java-maven-app` Git repository.
Within the `simple-java-maven-app` directory, run the commands:
+
.. `git add .`
.. `git commit -m "Add initial Jenkinsfile"`
.. `git push origin master`
. NOTE: you might need to log in to use git to push.

. In Jenkins, sign in if necessary, Go to your multibranch pipeline created earlier and select *Scan Multibranch Pipeline Now* on the left pane.
. This will scan your repo again for Jenkinsfile and this time it will find it. and Give a `SUCCESS` message again.
. Select *Status* from the left pane run options, then you should be able to see a job running named as *master*, click on it.
. It should be buiding the job with `checkout SCM` and `Build` section.
...
. In Jenkins, sign in if necessary, Go to your multibranch pipeline created earlier and select *Status* on the left pane.
. 
. Select *master* from the right pane run options, then you should be able to see *Buid Now*, click on it.

The interface displays as below if a Pipeline runs successfully.
